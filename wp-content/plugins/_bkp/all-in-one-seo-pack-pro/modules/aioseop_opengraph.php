	...
	class All_in_One_SEO_Pack_Opengraph extends All_in_One_SEO_Pack_Module {
		...
		function add_meta() {
			...
			if ( is_front_page() ) {
				...
				//$desc_from_main_settings  = trim( strip_tags( get_post_meta( $post->ID, 'description', true ) ) );
				...
				if ( empty( $description ) ) {
					...
					//$description = trim( strip_tags( get_term_meta( $term_id, 'description', true ) ) );
				}
				// Add default title.
				if ( empty( $title ) ) {
					$title = get_the_title();
				...
				if ( empty( $description ) && ! post_password_required( $post ) ) {
                    ...
                    //}
                    //var_dump(get_option('cat_meta_key_'.));
                    //var_dump(get_option($current_taxonomy.'_meta_key_'.$term_id));
                    //$description = trim( strip_tags( get_term_meta($term_id ? $term_id : get_queried_object()->term_id, 'description', true ) ) );
                    //$description = trim( strip_tags( get_woocommerce_term_meta($term_id ? $term_id : get_queried_object()->term_id, 'description', true ) ) );
                    //if(empty($description)) {

                        //$description = trim( strip_tags( get_woocommerce_term_meta($term_id ? $term_id : get_queried_object()->term_id, 'tag_desc', true ) ) );
                    //}
			...
			// Apply last filters.
			$metas = apply_filters( 'aioseop_description_title', $metas );

            $description = $metas['description'];
            $title = $metas['title'];
            $image = $metas['image_url'];
