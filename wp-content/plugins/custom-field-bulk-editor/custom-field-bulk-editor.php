...
function cfbe_editor() {
	...
	$args = array(
		"post_type" => $post_type,
		"posts_per_page" => isset($_GET['posts_per_page']) ? (int)$_GET['posts_per_page'] : 2000,
	...
	<script>
		...
		function filterFunction() {
			...
			input = document.getElementById('effectikInput');
	...
	foreach ($taxonomies AS $taxonomy) {
		...
		if($taxonomy === 'category') {
			echo '<input type="text" placeholder="Фильтровать" id="effectikInput" onkeyup="filterFunction()">';
		}
		...
		$.fn.ajaxBroker = function($el) {
			...
			$.ajax({
				...
				'beforeSend': function( xhr ) {
					$("#cfbe_multi_fieldvalue_" + postid).css("border","3px solid blue");
				},
				'error': function(xmlhttprequest, textstatus, message) {
					...
					$("#cfbe_multi_fieldvalue_" + postid).css("border","3px solid red");
				}  			
			}).done(function(response) {
				$("#cfbe_multi_fieldvalue_" + postid).val(response).css("border","3px solid green");
			}).fail(function(){
				$("#cfbe_multi_fieldvalue_" + postid).css("border","3px solid red");
			});
		...
		$(".fill_down").click(function() {
			...
			$(".cfbe-table > tbody > tr").each(function() {
				...
				if (parseInt(this_rel) >= parseInt(parent_rel)) {
					...
					var postid = $(this).find(".cfbe_multi_fieldname").data("postid");
					...
					var params = {
						'action': 'cfbe_lookup_meta_value',
						'post_id': postid,
						'field_name': $(this).find(".cfbe_multi_fieldname").val()
					};
					...
					ajaxManager.addReq({
						...
						success: function(data){
							$("#cfbe_multi_fieldvalue_" + postid).css("border","3px solid green").val(data);
						}
					});
					// $.fn.ajaxBroker($(this).find(".cfbe_multi_fieldname").val(fieldname));
				}
			});
...
function cfbe_lookup_meta_value_callback() {
	...
	// $data = $wpdb->get_results("SELECT `meta_value` FROM $wpdb->postmeta WHERE `post_id` = {$_POST['post_id']} AND `meta_key` LIKE '{$_POST['field_name']}%'");
