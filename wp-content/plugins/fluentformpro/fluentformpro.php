...
define('FLUENTFORMPRO_VERSION', '4.3.5');
...
if (!class_exists('FluentFormPro')) {
    ...
    class FluentFormPro
    {
        ...
        public function maybeDecryptHooks()
        {
            $fileTypes = ['input_file', 'input_image', 'featured_image'];

            foreach ($fileTypes as $fileType) {
                add_filter('fluentform_input_data_' . $fileType, function($files) {
                    ...
                    foreach ($files as $file) {
                        ...
                        $decrypted[] = str_replace($match[0], \FluentFormPro\Protector::decrypt($match[0]), $file);
                    }
