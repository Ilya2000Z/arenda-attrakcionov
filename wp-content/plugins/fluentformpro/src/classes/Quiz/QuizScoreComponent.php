...
class QuizScoreComponent extends BaseFieldManager
{
    ...
    public function addScoreToSubmission($value, $field, $submissionData, $form)
    {
        ...
        $settings = $quizController->getSettings($form->id);
        $quizFields = $settings['saved_quiz_fields'];

        $quizResults = $quizController->getFormattedResults($quizFields, $submissionData, $form);
        $score = 0;
        $totalCorrect = 0;
        $totalPoints = 0;
        ...
        $notGraded = apply_filters('fluentform_quiz_no_grade_label', __('Not Graded', 'fluentformpro'));
        ...
        switch ($scoreType) {
            ...
            case 'fraction_point':
                ...
                break;
            case 'percent':
                $result = round(($score / $totalPoints), 2) . '%';
                break;
            case 'grade':
