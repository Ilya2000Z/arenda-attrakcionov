...
class QuizController
{
    ...
    public function init($app)
    {
        ...
        add_filter('fluentform_shortcode_parser_callback_quiz_result', [$this, 'getQuizResult'], 10, 2);
     ...
     * @return string|void
     */
    public function getQuizResult($shortCode, ShortCodeParser $parser)
    {
        $form = $parser::getForm();
        $entry = $parser::getEntry();
        $settings = $this->getSettings($form->id);

        if ( ! $entry || ! $form || $settings['enabled'] != 'yes') {
            return;
        }
        $quizFields = $settings['saved_quiz_fields'];
        $response = json_decode($entry->response, true);
        $results = $this->getFormattedResults($quizFields, $response, $form);
        if (empty($results)) {
            return;
        }
        $rightIcon = self::getRightIcon();
        $wrongIcon = self::getWrongIcon();
        ...
        foreach ($results as $result) {
            ...
            $img = $isCorrect ? $rightIcon : $wrongIcon;

            $html .= sprintf("<tr class=\"field-label\"> <td style=\"display:flex;align-items: center; padding: 6px 12px; background-color: #f8f8f8; text-align: left;\"><div style='width: 20px;margin-right: 10px;'>  %s </div> <div><b>%s </b></div></td></tr>",
                $img, $result['label']);


            $userValueFormatted = is_array($result['user_value']) ? join(', ',
                $result['user_value']) : $result['user_value'];
            ...
            $correctAnsFormatted = is_array($result['correct_value']) ? join(', ',
                $result['correct_value']) : $result['correct_value'];
            if ( ! $isCorrect) {
                $conditionText = '';
    ...
    protected function getQuizFields($form)
    {
        ...
        foreach ($fields as $name => $value) {
            ...
            $fields[$name]['advance_points'] = (object)[];
        }
        ...
        switch ($element) {
            ...
            case 'input_checkbox':
                ...
                if ($condition == 'equal') {
                    ...
                    $isCorrect = count(array_intersect($userValue,
                            $correctValue)) && count($correctValue) == count($userValue);
     ...
     * @return array
     */
    public function getFormattedResults($quizFields, $response, $form)
    {
    ...
    public function pushQuizResult($cards, $entryData)
    {
        ...
        $widgets['quiz_result'] = $widgetData;
