...
$(function(){
    ...
    // })
    // })
    // $('#the_circle ul li a').mouseleave(function(){
    // $('.c_').stop();
    // $('.c_').animate({'opacity':'0'}, 100, function(){
    ...
    $('.a_').mouseenter(function () {
        ...
        //var id2 = id.replace('c_','l_');
        $('#' + id).animate({'opacity': '1'}, 100, function () {
            //$('#'+id2).children('a').addClass('hovered');
        })
    })
    $('.a_').mouseleave(function () {
        $('.c_').stop();
        //$('.c_').animate({'opacity':'0'}, 10, function(){
        //$('#the_circle ul li a').removeClass('hovered');
        //})
        $('.c_').css('opacity', '0');
        //$('#the_circle ul li a').removeClass('hovered');
    })
	...
	/* Изменения №1, обработчик события click на ".go_description", опускаем экран */
	$('.go_description').click(function(event){
		$('html, body').animate({ scrollTop: ($('div#item_block').offset()).top-150}, 'slow');
	})
	/* Конец изменения №1, обработчик события click на ".go_description", опускаем экран */
    ...
    $('.items_list .item .pic').click(function () {
        //var url = $(this).parent().children('.cpt').children('a').attr('href');
        //window.location = url;
    })
    ...
    $('#pics .previews li').mouseleave(function () {
        //$('#pics .big_one .over').fadeOut(200, function(){
        $('#pics .big_one .over').remove();
        //});
    })
    $('#pics .previews li').click(function () {
        /*var path = $(this).children('img').attr('src');
        $('#popup tr td img').attr('src', path);
        $('#overlay').fadeIn(200, function () {
            $('#popup').fadeIn(200);
        })*/    /* Изменения №2 (галерея ,закоментировал старый обработчик при нажатие на круг с фотографией) */


		/* Изменения №2 (галерея, добавил, новый обработчик, при нажатие на круг с фотографией открывается галерея) */
		$('#overlay').fadeIn(200, function () {
		...
		//console.log(number_li);
		$(number_li).click();
		/* Конец изменения №2 (галерея, добавил, новый обработчик, при нажатие на круг с фотографией открывается галерея) */
    })


    $('#overlay, #popup, #close_galery').click(function () { /* Изменения №2 (добавил "#close_galery, #video_galery") */
...
*/
    ...
    /*
    $('.items_list .item').on('click', function(){

        $.cookie('item_source', $( this ).data('src'), {
                        expires: 5,
                        path: '/',
                    });
    ...
    */
    ...
    /*
    var wwwww = $(document).width();
    if (wwwww <= 1170) {
        $('.items_list').each(function () {
            $(this).children('.item:nth-child(4n)').next('.item').css('clear', 'none');
            $(this).children('.item:nth-child(3n)').next('.item').css('clear', 'both');
        })
    }
    else {
        $('.items_list').each(function () {
            $(this).children('.item:nth-child(4n)').next('.item').css('clear', 'both');
            //$(this).children('.item:nth-child(3n)').next('.item').css('clear','none');
        })
    }
    */
...
var admin_bar_height={
    ...
    change_heigth:function(){
       ...
       /* if(this.bar==='' || this.header===''){
            //console.log ('Функция admin_bar_height не получила нужных параметров (ниже приведены значения)');
            //console.log ('Значение высоты админ-бара = '+ (this.bar===''?'Пустое значение':this.bar));
            //console.log ('Значение top шапки = '+ (this.header===''?'Пустое значение':this.header));
            return;
        }
        */
...
//изменения бордера сёрча при фокусе
...
$(function scroll_menu(){
    if( !( $('body').hasClass('category-305') ) ) {
    	$(window).scroll(function() {
    		...
    		var height_left_menu=$('#cat_menu').height();

    		if ((document_height-top) < (height_footer+height_left_menu)+130)
    		{
    			$("#cat_menu").css({top: document_height-height_left_menu-height_footer-130, position: 'absolute'});
    			//alert('test');
    		}
    		else {
    		   ...
    		   $("#cat_menu").css({top: top_menu+4+'px', position: 'fixed'});
    		}
            /*if(top > 150){

                $(".filter-table").addClass('fixed');

            }
            else {
                $(".filter-table").removeClass('fixed');
            }
             */
    	});
    }
